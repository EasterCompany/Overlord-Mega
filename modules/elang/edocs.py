...
class eDoc:
    ...
    def __init__(self, route):
        self.ROUTE = route
        self.HTML_FILES = []		# HTML FILES INCLUDED IN THIS EDOC
        self.CSS_FILES = [] 		# CSS  FILES INCLUDED IN THIS EDOC
        self.JS_FILES = []			# JS   FILES INCLUDED IN THIS EDOC
        self.SITE_APPS = [] 		# EDOC CLASS INCLUDED IN THIS EDOC
        self.SITE_STYLE = False 	# DEFAULT SITE CSS STYLE FILE DIR
        self.LOCAL_BUILD = open("templates/pages/" +
                                route + "/" + route + ".html").read()
        # Remove redundent spacing
        while "  " in self.LOCAL_BUILD:
            self.LOCAL_BUILD = self.LOCAL_BUILD.replace("  ", " ")
        self.compile()
    ...
    def compile(self):
        ...
        if "debug" in argv:
            self.LOCAL_BUILD = OUTPUT
        # When in production enviroment
        self.LOCAL_BUILD = OUTPUT.replace("\n", "").replace("\r", "")
