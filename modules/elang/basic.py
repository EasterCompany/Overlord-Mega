...
class TestSuite:
  ...
  def __init__(self, tests, _name="__TestSuite__", _post=True, _eject_on_fail=True):
    self.post = _post
  ...
  UNIT TEST """ + name + """
  { """ + str(tar[0]) + """ }
  { """ + str(tar[1]) + """ } """ + str(fin).replace("=>", "=> " + str(res) + " ?")

    def this_test(t=test_func[0](test_func[1]), r=expectation):
...
def __unit_test__(eject_on_fatal=True):
